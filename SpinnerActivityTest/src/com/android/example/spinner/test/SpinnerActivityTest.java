...
public class SpinnerActivityTest extends
	...
	/*
	 * The Spinner object in the app under test. Used with instrumentation to
	 * control the app under test.
	 */
	...
	public void testPreconditions() {
		...
		/*
		 * An example of an initialization test. Assert that the item select
		 * listener in the main Activity is not null (has been set to a valid
		 * callback)
		 */
	...
	/*
	 * Tests the UI of the main activity. Sends key events (keystrokes) to the
	 * UI, then checks if the resulting spinner state is consistent with the
	 * attempted selection.
	 */
	public void testSpinnerUI() {
		...
		/*
		 * from the spinner's data mLocalAdapter, get the object at the selected
		 * position (this is a String value)
		 */

		mSelection = (String) mSpinner.getItemAtPosition(mPos);

		/*
		 * Get the TextView widget that displays the result of selecting an item
		 * from the spinner
		 */
		...
		/*
		 * Confirm that the EditText contains the same value as the data in the
		 * mLocalAdapter
		 */
	...
	/*
	 * Tests that the activity under test maintains the spinner state when the
	 * activity halts and then restarts (for example, if the device reboots).
	 * Sets the spinner to a certain state, calls finish() on the activity,
	 * restarts the activity, and then checks that the spinner has the same
	 * state.
	 */
	...
	public void testStateDestroy() {
		...
		/*
		 * Set the position and value of the spinner in the Activity. The test
		 * runner's instrumentation enables this by running the test app and the
		 * main app in the same process.
		 */
		...
		// Restart the activity by calling
	...
	/*
	 * Tests that the activity under test maintains the spinner's state when the
	 * activity is paused and then resumed.
	 * 
	 * Calls the activity's onResume() method. Changes the spinner's state by
	 * altering the activity's View. This means the test must run on the UI
	 * Thread. All the statements in the test method may be run on that thread,
	 * so instead of using the runOnUiThread() method, the
	 * 
